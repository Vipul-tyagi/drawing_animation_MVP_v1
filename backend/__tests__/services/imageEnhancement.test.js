  ...
  beforeEach(() => {
    ...
    mockS3ClientInstance = {
      send: jest.fn((command) => {
        // console.log('S3Client.send called with command:', command.constructor.name); // Log the command type
        if (command instanceof GetObjectCommand) {
          ...
          // console.log('S3Client.send returning for GetObjectCommand:', mockResponse); // Log the response
          return Promise.resolve(mockResponse);
        } else if (command instanceof PutObjectCommand) {
          const mockResponse = {};
          // console.log('S3Client.send returning for PutObjectCommand:', mockResponse); // Log the response
          return Promise.resolve(mockResponse);
        }
        // console.log('S3Client.send rejecting for unknown command:', command); // Log unknown commands
